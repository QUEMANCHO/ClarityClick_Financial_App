-- =========================================================================================
-- MIGRACIÓN FASE 3: Refinamiento del Cerebro (Vista SQL)
-- 
-- Objetivo: Actualizar el cálculo de `v_estrategia_metas_detalle` para que la capacidad
-- financiera del usuario NO incluya aquellos ingresos que el usuario descartó (marcó 
-- como que "son para otra cosa" / excluir_de_ia = true).
-- =========================================================================================

-- Reemplazamos la vista
...
WITH 
  ...
  global_stats AS (
      SELECT 
          user_id,
          -- Aquí está el cambio: Solo sumamos 'Ganar' si NOT excluir_de_ia
          SUM(CASE WHEN pilar = 'Ganar' AND excluir_de_ia = false THEN cantidad ELSE 0 END) as total_ganado,
          SUM(CASE WHEN pilar = 'Gastar' THEN cantidad ELSE 0 END) as total_gastado,
          -- Contamos los meses activos usando solo transacciones
          GREATEST(COUNT(DISTINCT date_trunc('month', fecha)), 1) as meses_activos
      FROM transacciones
      GROUP BY user_id
  ),
  user_metrics AS (
      SELECT 
          user_id,
          COALESCE(total_ganado / meses_activos, 0) as promedio_ingresos_mensual,
          COALESCE(total_gastado / meses_activos, 0) as promedio_gastos_mensual,
          -- Capacidad Real Bruta (Promedio)
          COALESCE((total_ganado - total_gastado) / meses_activos, 0) as capacidad_ahorro_mensual
      FROM global_stats
  ),
  -- Normalización de aportes de cada meta a mensual (AÑADIDO DESDE V2)
...
SELECT 
    ...
    -- CAPACIDAD NETA DISPONIBLE (AÑADIDO DESDE V2)
